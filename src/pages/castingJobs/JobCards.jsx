    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-[20px] shadow-2xl w-full max-w-sm mx-4 p-6 flex flex-col gap-4">
        ...
        <div className="text-center">
          <h2 className="text-[18px] font-bold text-gray-800 mb-1">Delete Job?</h2>
          <p className="text-[13px] text-gray-500">
            Are you sure you want to delete{" "}
            <span className="font-semibold text-gray-700">"{job.job_title}"</span>?
...
const StaticJobCard = ({ setJobUid }) => {
  ...
  const paginatedJobs = jobs.slice(pageIndex * JOBS_PER_PAGE, (pageIndex + 1) * JOBS_PER_PAGE);

  const handleMenuOpen = (event, job) => { setAnchorEl(event.currentTarget); setSelectedJob(job); };
  const handleMenuClose = () => { setAnchorEl(null); setSelectedJob(null); };
  ...
  const handleDeleteClick = () => { setDeleteModalJob(selectedJob); handleMenuClose(); };
  ...
  if (loading) return <div className="flex justify-center py-24"><p className="text-gray-400">Fetching your jobs...</p></div>;
  if (jobs.length === 0) return <div className="flex justify-center py-24"><p>No jobs created yet.</p></div>;
  ...
  return (
    <>
      ...
      <EditJobModal
        ...
        onUpdated={() => { setEditModalJob(null); fetchJobs(); }}
      />
      ...
      <div className="flex flex-col">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-4">
          {paginatedJobs.map((job) => {
            ...
            return (
              <div key={job.uuid} className="bg-white rounded-[22px] shadow-md h-[360px] flex flex-col">
                <div className="p-[22px] flex flex-col flex-1">
                  <div className="flex items-center justify-between">
                    <h1 className="text-xs text-gray-400">Job Title</h1>
                    <IconButton size="small" onClick={(e) => handleMenuOpen(e, job)} sx={{ color: "gray" }}>
                      <MoreVertIcon fontSize="small" />
                    </IconButton>
                  </div>
                  <h2 className="text-[20px] font-bold mt-2 mb-2 line-clamp-2">{job.job_title}</h2>
                  <p className="text-[13px] text-[#555] line-clamp-3">{job.job_description}</p>
                  <p className="text-[11px] text-[#888] mt-3">Posted {job.job_sub_date || "Recently"}</p>
                  <div className="flex flex-wrap gap-2 mt-3">
                    {tags.map((tag, i) => (
                      <span key={i} className="bg-gray-100 px-2 py-1 rounded-full text-[11px]">{tag}</span>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="text-[16px] font-bold text-primary">{job.total_applicants}</div>
                      <div className="text-[11px] text-gray-500">Total Applicants</div>
                    </div>
                    <button
        ...
        <Menu
          ...
          PaperProps={{ sx: { borderRadius: "12px", minWidth: 140, boxShadow: "0px 4px 20px rgba(0,0,0,0.1)" } }}
        >
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex justify-between px-4">
            <button onClick={() => setPageIndex((p) => p - 1)} disabled={pageIndex === 0}>Previous</button>
            <span>{pageIndex + 1} / {totalPages}</span>
            <button onClick={() => setPageIndex((p) => p + 1)} disabled={pageIndex === totalPages - 1}>Next</button>
          </div>
        )}
      </div>
    </>
